...
os_tab2 = tidyr::extract(os_tab, fusion, c("left", "right"), "(.*)-(.*)") # the second argument is not "fusion" but the name of the column
...
################## 
# the issue with the above is that it only takes into consideration by disease, here is a breakdown irrespective of disease
# still not much dominatge though
fusion_inframe_tab_right = data.frame ( table ( fuse2.1 [ fuse2.1$frameness == "in.frame",  ]$right ))
fusion_inframe_tab_right= fusion_inframe_tab_right[ order ( -fusion_inframe_tab_right$Freq), ]
...
test_is_cancer = fuse2[(fuse2$frameness == "in.frame" | fuse2$frameness == "out.of.frame") & fuse2$is.cancer == "yes", ] %>%
  group_by ( frameness, RNAseq.id) %>% summarise( total = n())

 x1 = test_is_cancer[test_is_cancer$frameness == "in.frame", ]$total
  x2 = test_is_cancer[test_is_cancer$frameness == "out.of.frame", ]$total
...
test.sample = fuse2.1 [ fuse2.1$right == test.gene & grepl ( "in.frame", fuse2.1$frameness), c("RNAseq.id", "fusion") ]
...
#test.sample = fuse2.1 [ grepl ( "COL" , fuse2.1$fusion) , c("RNAseq.id", "fusion") ]
...
test.tpm = merge ( test.tpm, trueseq.key[trueseq.key$cancer.type %in% ctype , c("RNAseq.id", "cancer.subtype") ], by.x="variable",  by.y="RNAseq.id" )
...
# because this involves outlier we need to now remove failedQC 
## rethinking outlier fusions by plotting median and the tpm instead 

#  temp.all has the type fixed while fuse2.1 has the framness fix. 
# note once I move the framness fix to the main fusion this will longer be necessary. 

up.all_qc = rna.up[!rna.up$RNAseq.id %in% failedChem, ]
down.all_qc = rna.down[!rna.down$RNAseq.id %in% failedChem, ] 

draw_fusionout <- function( fuse2.2, framethis = "in.frame", qcout) {
  
fuse_out2 = fuse2.2[ ! fuse2.2$RNAseq.id %in% failedChem, ]
fuse_out2$right = gsub ( ".*-", '',fuse_out2$fusion)
fuse_out2 = merge ( fuse_out2, qcout, by.x=c ("right","RNAseq.id"), c( "gene","RNAseq.id" ) )
fuse_out2 = fuse_out2[  fuse_out2$frameness %in% framethis,  ]
...
fuse_out2$fusion = factor ( fuse_out2$fusion, levels = unique ( fuse_out2$fusion) )





# combine is.diag with is.obsv 

fuse_out2$type2 = gsub ("is.diag|is.obsv", "is.diag/obsv",    fuse_out2$type)
...
fuse_outlier = ggplot( fuse_out2  , aes(x=reorder_within (  RNAseq.id, delta ,  fusion) , y=delta, fill=type   )) +
  ...
  coord_flip() + ggtitle ( "In-frame Fusion  + 3' as Outliers") +
  scale_y_continuous(expand = expansion(mult = c(0, .1))) +
  facet_wrap(. ~ type2, scales = "free" , ncol=3 ) +
  theme(
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) +
  geom_text(
...
fuse2.2 = merge ( fuse2.1, temp.all[ , c("uid", "type")], by= "uid")

fuse_outlier = draw_fusionout(fuse2.2, "in.frame", qcout = up.all_qc)
# there are zero out.of.frame fusions that 3' is an outlier. 
# fuse_outlier_down_outframe = draw_fusionout(fuse2.2, framethis = "out.of.frame", qcout = down.all_qc )
...
right_rnk = oncofuse  %>% 
  ...
  dplyr::summarise  (
    Freq = n()
  )  %>% arrange ( desc ( Freq), desc (right) ) %>% data.frame ( stringsAsFactors = F) %>% .$right 

left_rnk = oncofuse  %>% 
  group_by( left )  %>%
  dplyr::summarise  (
    Freq = n()
  )  %>% arrange ( desc ( Freq) , desc (left) ) %>% data.frame ( stringsAsFactors = F) %>% .$left 
...
# important using "role" which means eg oncogene = oncogene + (oncogene,fusion) but not (oncogene,tsg)
# tabulation of outliers  

combolist=genelist$gene
this.up_tab = up.all_qc [ up.all_qc$gene %in% unique ( c ( combolist) ),  ]
# need to remove drug becase this should only be for tsg's 
this.down_tab = down.all_qc [ down.all_qc$gene %in% unique ( c ( combolist) ),  ]


this.up_tab = merge ( this.up_tab, rna.key[ , c("RNAseq.id", "cancer.subtype")], by="RNAseq.id")

this.up_tab2 =  this.up_tab[this.up_tab$role %in%  c (  "oncogene") ,  ]   %>% dplyr::group_by (cancer.subtype, gene) %>%
...
```{r}
top_kinase # what happend to +top_kinase_family
```
...
nocancer_tab_f2 = nocancer_tab_f[nocancer_tab_f$total >=6 | nocancer_tab_f >= 10, ]
...
# which genes are present in more than 6 or more cancer subtypes? 
cancer_tab_f2 = cancer_tab_f[cancer_tab_f$total >=3 | cancer_tab_f$total_freq >= 10 , ]
...
temp = up.all_qc
temp = merge ( temp, rna.key[, c("RNAseq.id", "cancer.subtype")], by="RNAseq.id")

table ( temp[ temp$gene == "PAX7", ]$cancer.subtype)
table ( temp[ temp$gene == "JAK1", ]$cancer.subtype)
table ( temp[ temp$gene == "COL2A1", ]$cancer.subtype)
```
...
waffle_drugs = waffle::waffle(level_data, rows=12 , size=1, 
...
# 5.14.2022 updating plots for outlier trends. 
# biopsy vs met ? 
...
# dont use outlier.both, probably doesnt matter but its confusing 

temp_qc.all = up.all_qc
temp_qc.all$type = "UP"
temp_qc.all2 = down.all_qc
temp_qc.all2$type = "DOWN"

outlier.both2= rbind ( temp_qc.all, temp_qc.all2 )

outlier.both2 = merge ( outlier.both2, trueseq.key[, c("RNAseq.id", "treatment", "sample.type", "sample.classification")], by="RNAseq.id")
outlier.both2$sample.type = gsub ( ".*_", "", outlier.both2$sample.type )

outlier.both2$sample.classification = gsub ( "NonPrimary" , "Metastasis", outlier.both2$sample.classification )
outlier.both2$sample.classification = gsub ( "Primary" , ".Primary", outlier.both2$sample.classification )
outlier.both2$sample.classification = factor ( outlier.both2$sample.classification, c(  ".Primary", "Metastasis"    ))

# remove unknown 

outlier.both2 = outlier.both2[outlier.both2$treatment != "Unknown",  ]
outlier.both2$type = ifelse ( outlier.both2$type == "UP", ".UP", "DOWN")
outlier.both2 = merge ( outlier.both2, genelist[ , c("gene", "cosmic.annot")], by="gene", all.x=T, all.y=FALSE)
outlier.both2[is.na(outlier.both2)] <- "no_cancer"

out_treat = outlier.both2 %>% dplyr::group_by( type, RNAseq.id, treatment) %>%
  dplyr::summarise(
    Total = log2 ( n() )  
  ) %>% dplyr::group_by( type, treatment) %>% arrange ( desc ( type) ) %>% 
  dplyr::mutate( Mean = mean ( Total) ,
                 #type = factor ( type, levels = c(".UP", "DOWN"))
                 ) %>% ggplot(. , aes(y=Total, x=treatment))+
  geom_quasirandom(aes(fill = treatment ), size = 6, shape = 21, alpha = .5) +
  theme_bw() +
  xlab("") +  ylab("log2 ( Total )") +
  theme(legend.position="none", legend.title=element_blank(), legend.key = element_blank(),
  ...
  scale_fill_manual(values = unique (brewer.pal(7, "Dark2") )) + facet_grid( type ~. , scales = "free" ) +
  theme(strip.text.y = element_text(size = 30))

out_treat = my_add_pval(out_treat, pairs = list(c(1, 2)), test='wilcox.test', textsize = 8, response = 'Total' )

out_treat # change facet size here.




out_treat_oncogene = outlier.both2[outlier.both2$cosmic.annot == "ONCOGENE", ] %>% dplyr::group_by( type, RNAseq.id, treatment) %>%
  dplyr::summarise(
    Total = n()  
  ) %>% dplyr::group_by( type, treatment) %>% arrange ( desc ( type) ) %>% 
  dplyr::mutate( Mean = mean ( Total) ,
                 #type = factor ( type, levels = c(".UP", "DOWN"))
                 ) %>% ggplot(. , aes(y=log2 ( Total), x=treatment))+
  geom_quasirandom(aes(fill = treatment ), size = 3, shape = 21, alpha = .5) +
  theme_bw() +
  xlab("") +
  theme(legend.position="none", legend.title=element_blank(), legend.key = element_blank(),
...
out_treat_oncogene = my_add_pval(out_treat_oncogene, pairs = list(c(1, 2)), test='wilcox.test', textsize = 10, response = 'Total' )

out_treat_oncogene



out_treat_tsg= outlier.both2[outlier.both2$cosmic.annot == "TSG", ] %>% dplyr::group_by( type, RNAseq.id, treatment) %>%
  dplyr::summarise(
    Total = n()  
  ) %>% dplyr::group_by( type, treatment) %>% arrange ( desc ( type) ) %>% 
  dplyr::mutate( Mean = mean ( Total) ,
...
out_treat_tsg = my_add_pval(out_treat_tsg, pairs = list(c(1, 2)), test='wilcox.test', textsize = 10, response = 'Total' )
...
out_class = outlier.both2 %>% dplyr::group_by( type, RNAseq.id, sample.classification) %>%
  dplyr::summarise(
    Total = log2 ( n() )  
  ) %>% dplyr::group_by( type, sample.classification) %>% arrange ( desc ( type) ) %>% 
  dplyr::mutate( Mean = mean ( Total) ,
                 #type = factor ( type, levels = c(".UP", "DOWN"))
                 ) %>% ggplot(. , aes(y=Total, x=sample.classification))+
  geom_quasirandom(aes(fill = sample.classification ), size = 6, shape = 21, alpha = .5) +
  theme_bw() +
  xlab("") +  ylab("log2 ( Total )") +
  theme(legend.position="none", legend.title=element_blank(), legend.key = element_blank(),
  ...
  scale_fill_manual(values = unique (brewer.pal(7, "Accent") )) + facet_grid( type ~. , scales = "free" ) +
  theme(strip.text.y = element_text(size = 30))

out_class = my_add_pval(out_class, pairs = list(c(1, 2)), test='wilcox.test', textsize = 8, response = 'Total' )

out_class # change facet size here.


# calculate entropy 
se = readRDS("/data/SUMMARY/TPM/shannonEntropy.tpm2.rds")
se = merge ( outlier.both2[!duplicated ( outlier.both2$RNAseq.id),  ],se, by.x="RNAseq.id", by.y="th_sampleid" )

sep_class = ggplot(se , aes(y=se, x=sample.classification))+
  geom_quasirandom(aes(fill = sample.classification ), size = 6, shape = 21, alpha = .5) +
  theme_bw() +
  ...
  scale_fill_manual(values = unique (brewer.pal(7, "Accent") ))  +
  theme(strip.text.y = element_text(size = 30))

sep_class = my_add_pval(sep_class, pairs = list(c(1, 2)), test='wilcox.test', textsize = 8, response = 'se' )

sep_class # change facet size here.


sep_treat = ggplot(se , aes(y=se, x=treatment))+
  geom_quasirandom(aes(fill = treatment ), size = 6, shape = 21, alpha = .5) +
  theme_bw() +
  xlab("") +  ylab(" Shannon Entropy  ") +
  theme(legend.position="none", legend.title=element_blank(), legend.key = element_blank(),
  ...
  scale_fill_manual(values = unique (brewer.pal(7, "Dark2") ))   +
  theme(strip.text.y = element_text(size = 30))

sep_treat = my_add_pval(sep_treat, pairs = list(c(1, 2)), test='wilcox.test', textsize = 8, response = 'se' )

sep_treat # change facet size here.
...
out_treat + ( out_class + ylab("")) + plot_layout(widths =c ( .5,.5 ) )
...
daf_vs_raf = ggplot(cosmic, aes(x=TUMOR.AF, y= RNA_ALT_AF)) + 
  geom_point( aes ( color=balance_simple , alpha=balance), position=position_jitter(h=0.01, w=0.01) , size=3.5  )  + 
...
# Examples of RAF  and DAFs

#### RAF > DAF cancer. 
raf_examples = cosmic[ cosmic$is.sig == "sig" & cosmic$is.cancer == "yes" & cosmic$balance == "EXTREME_HIGH" & cosmic$VEP_Worst_Impact_simple == "High/Moderate", ] 
 head ( raf_examples[ , c(  "RNAseq.id", "cancer.subtype" , "chrom","pos" ,"ref", "alt"  , "RNA_DEPTH" , "RNA_REF_COUNT"    , "RNA_ALT_COUNT" ,  "RNA_ALT_AF" , "TUMOR.AF", "RNA_PVALUE"  , "VEP_Worst_Impact_simple" , "VEP_Worst_Gene", "CLINVAR_MATCH", "COSMIC_MATCH" )] )
 
 temp = raf_examples[ , c(  "RNAseq.id", "cancer.subtype" , "chrom","pos" ,"ref", "alt", "RNA_DEPTH" , "RNA_REF_COUNT"    , "RNA_ALT_COUNT" ,  "RNA_ALT_AF" , "TUMOR.AF"  , "RNA_PVALUE"  , "VEP_Worst_Impact_simple" , "VEP_Worst_Gene" , "CLINVAR_MATCH", "COSMIC_MATCH")] 

 temp  =  data.frame ( lapply ( temp, function (x) if(is.numeric(x)) { round ( x, 2)} else x  )  )
 colnames ( temp)[13:14] = c("impact","gene")
 kable ( temp[grepl("TP53|ABL1|COL|NRAS", temp$gene) , ]
        , format="html", row.names=F, table.attr = "style='width:10%;'" ) %>%
  kable_classic(full_width = F, position = "center",)  %>%
  column_spec(c(10,11), background = "#d9dbdb")  %>%
  column_spec(c(5,6), background = "#98b2b8")  

#### RAF > DAF not cancer 
raf_examples = cosmic[ cosmic$is.sig == "sig" & cosmic$is.cancer == "no" & cosmic$balance == "EXTREME_HIGH" & cosmic$VEP_Worst_Impact_simple == "High/Moderate", ] 
 head ( raf_examples[ , c(  "RNAseq.id", "cancer.subtype" , "chrom","pos" ,"ref", "alt"  , "RNA_DEPTH" , "RNA_REF_COUNT"    , "RNA_ALT_COUNT" ,  "RNA_ALT_AF" , "TUMOR.AF", "RNA_PVALUE"  , "VEP_Worst_Impact_simple" , "VEP_Worst_Gene" , "CLINVAR_MATCH", "COSMIC_MATCH" )] )
 ...
 temp  =  data.frame ( lapply ( temp, function (x) if(is.numeric(x)) { round ( x, 2)} else x  )  )
 colnames ( temp)[13:14] = c("impact","gene")
temp = temp [ order ( -temp$RNA_ALT_AF), ]
test_this = c("CDC42EP2", "MED24","CSMD1", 'VWA2', 'COL1A1')
  kable ( temp[temp$gene %in% test_this  & temp$COSMIC_MATCH == "", ]
        , format="html", row.names=F, table.attr = "style='width:10%;'" ) %>%
  kable_classic(full_width = F, position = "center",)  %>%
  column_spec(c(10,11), background = "#d9dbdb")  %>%
  column_spec(c(5,6), background = "#98b2b8")   
...
portion_consequence_stack_vert = ggplot(ap2 , aes(fill=balance_simple, y=Freq, x=synonymous ) )+ 
    geom_bar(position="fill", stat="identity", width=1, color='#e8eded') +
  # add text but position needs to be set to the same, which is filled
...
return ( list ( ht=ht_long_fusion, long_info_details = long_info, singleg =long_total_fusion, temporal.key=temporal.key , modkey =fusion_total2, long_out=long_out  ))


}

# update 5.16.2022
# take from compute_longi and plot single pid. 
longi_pid <- function ( dff, pid , gtitle = ""){
  
  # draw table 
  
  temp =  dff$long_info_details [ dff$long_info_details$pid == pid, ]

  table_1 <- kable ( temp
        , format="html", row.names=F, table.attr = "style='width:10%;'" ) %>%
  kable_classic(full_width = F, position = "center",)  %>%
  column_spec(1, bold = T, border_right = T)  %>% 
  row_spec( which ( temp$type == "primary_only" ), color = "black", background = "#9fbad6") %>%
  row_spec( which ( temp$type == "secondary_only" ), color = "black", background = "#2377cf") %>%
  row_spec( which ( temp$type == "both" ), color = "black", background = "#68717a") 

  t1 = dff$modkey[dff$modkey$patient.id == pid,  ]
  ...
  ggplot( . , aes(x = temporal.order, y = total_fusion, color = cancer.subtype, group = patient.id)) + 
  geom_point(size=6 , alpha = 0.75    ) + geom_line() + ylab(" total ")+ xlab("temporal.order: RNA-seq") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))  +
  theme(legend.position="none", legend.title=element_blank(), legend.key = element_blank(),
  ...
  scale_y_continuous(limits = c(0, NA) ) + ggtitle(gtitle)
  
return ( list ( plot = gplot1 ,
                table = table_1
                )
                )  
...
# draw table for single longi 
# assume key has the following:  "RNAseq.id" ,"patient.id"  ,"sex"  , "diagnosis"   ,   "sample.type"  ,  "treatment"  , "temporal.order"


draw_key_longi <- function ( pid , rkey ){
  
  temp = rkey[ rkey$patient.id == pid, c( "RNAseq.id" ,"patient.id"  ,"sex"  , "diagnosis"   ,   "sample.type"  ,  "treatment"  , "temporal.order" )]
temp = temp [ order ( temp$temporal.order), ]
temp = kable ( temp
        , format="html", row.names=F, table.attr = "style='width:10%;'" ) %>%
  kable_classic(full_width = F, position = "center",)  %>%
  column_spec(1, bold = T, border_right = T)  
...
# output can be used to draw single or entire cohort. 

fuse_longout = compute_longi(fuse2.1, "fusion")
outUP_longout = compute_longi( up.all_qc[ up.all_qc$both == "OUTLIER_HIGH" & up.all_qc$gene %in%  genelist$gene, ] , "gene" )
outUP_longout_nocancer = compute_longi( up.all_qc[ up.all_qc$both == "OUTLIER_HIGH" & !up.all_qc$gene %in%  genelist$gene, ] , "gene" )

outDOWN_longout = compute_longi( down.all_qc[ down.all_qc$both == "OUTLIER_LOW" & down.all_qc$gene %in%  genelist$gene, ] , "gene" )
outDOWN_longout_nocancer = compute_longi( down.all_qc[ down.all_qc$both == "OUTLIER_LOW" & !down.all_qc$gene %in%  genelist$gene, ] , "gene" )



# example of single draw 
outUP_longout_1 = longi_pid(dff=outUP_longout, pid="728", gtitle = "Outlier UP, Cancer Genes")
draw_key_longi("728", rna.key)

# draw cohort. 

draw ( outUP_longout$ht)
# create pie chart for it. 
tally = data.frame ( table ( outUP_longout$long_info_details$type )) 
tally$Var1 = gsub (  "both", "shared", tally$Var1)
# https://colordesigner.io/gradient-generator
outUP_cancerOnly_pie = make.pie2(
  ftype.freq = tally 
  ,name.first = "Type" # label for legend 
  , title = "UP Cancer Genes" 
  , cc =  c( "primary_only" = '#2c7ac2', 'secondary_only' ='#1f628b' , shared = '#848586') 
  , leg.pos = "none"
  ,pietext= 10
  ,legtext = 20 
  , dir = -1 
...
draw ( outUP_longout_nocancer$ht)

tally = data.frame ( table ( outUP_longout_nocancer$long_info_details$type )) 
tally$Var1 = gsub (  "both", "shared", tally$Var1)

outUP_non_cancer_pie = make.pie2(
  ftype.freq = tally 
  ,name.first = "Type" # label for legend 
  , title = "UP Non-Cancer Genes" 
  , cc =  c( "primary_only" = '#2c7ac2', 'secondary_only' ='#1f628b' , shared = '#848586') 
  , leg.pos = "bottom"
  ,pietext= 10
  ,legtext = 20 
...
tally = data.frame ( table ( outDOWN_longout$long_info_details$type )) 
tally$Var1 = gsub (  "both", "shared", tally$Var1)

outDOWN_longout_cancer_pie = make.pie2(
  ftype.freq = tally 
  ,name.first = "Type" # label for legend 
  , title = "UP Non-Cancer Genes" 
  , cc =  c( "primary_only" = '#2c7ac2', 'secondary_only' ='#1f628b' , shared = '#848586') 
...
outUP_cancerOnly_pie /  outUP_non_cancer_pie

# do a wilcox test. 


temps = outUP_longout$long_info_details 
temps$type.x = gsub ( "both", "shared", temps$type )
temps$type.x  = factor ( temps$type.x , levels = c( "primary_only", "shared", "secondary_only" ))


long_cmp_outCancer <- temps %>% group_by(type.x, pid) %>% summarise ( TotalbyType = n()
) %>%
...
long_cmp_outCancer = my_add_pval(long_cmp_outCancer, pairs = list(c(1, 2)  ), test='wilcox.test', post=1, textsize = 10)
long_cmp_outCancer = my_add_pval(long_cmp_outCancer, pairs = list(c(2, 3)  ), test='wilcox.test', post=1, textsize = 10) 

long_cmp_outCancer = long_cmp_outCancer+ggtitle ( "Outliers UP Cancer Genes")



# figure the highe freq genes in shared. 

gethigh_freq <- function(df , min = 3){
  
  df$type = gsub ( "both", "shared", df$type)
  df = data.frame ( table ( df [ , c("type", "info")] )) 
  df = df[ order ( -df$Freq), ] 
  df = df[df$Freq > min, ] 

temp = rbind ( head ( df[ df$type == "shared", ], 10 ) , head ( df[ df$type == "secondary_only", ], 10 ) )
temp = rbind ( temp, head ( df[ df$type == "primary_only", ], 10 ) )
...
cancer_only_table = gethigh_freq ( df=outUP_longout$long_info_details )
non_cancer_table = gethigh_freq(df=outUP_longout_nocancer$long_info_details)


faceted_longi_UP <- outUP_longout$modkey  %>% 
  dplyr::group_by(patient.id, temporal.order ) %>% 
  dplyr::mutate(
    total_fusion = median(total_fusion)
  ) %>% 
  ...
  theme(legend.position="none", legend.title=element_blank(), legend.key = element_blank(),
        ...
        legend.text      =element_text(size=12)
            ...
            axis.line = element_line(colour = "black"))+ scale_color_manual(values= subcolor) +
     facet_wrap(   cancer.subtype  ~. , scale="free") +   scale_x_continuous(  breaks=c(1, 2) ) +
  scale_y_continuous(limits = c(0, NA) ) + theme(strip.text.x = element_text(size = 15, colour = "black", angle = 0))
   

se_long= readRDS("/data/SUMMARY/TPM/shannonEntropy.tpm.rds")
se_long = merge ( outUP_longout$modkey, se_long, by.x="RNAseq.id", by.y = "th_sampleid" ) 
...
se_long %>% 
  ...
  ggplot( . , aes(x = temporal.order, y = mse, color = cancer.subtype, group = patient.id)) + 
  geom_point(size=6 , alpha = 0.75    ) + geom_line() + ylab(" total ")+ xlab("temporal.order: RNA-seq") +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))  +
...
# good way to find what is the most frequently upregulated in secondary. 
...
ggplot( .  , aes(y=TotalbyType, x=type.x))+
  ...
  scale_fill_manual(values = unique (brewer.pal(7, "Dark2") ))  + ggtitle("Gene Fusion")
