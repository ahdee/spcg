...
kable( tally  , format = "html" , row.names = F, caption="multi.sample" ) %>% kable_classic(full_width = F, position = "center", ) %>% row_spec(row= nrow ( tally), bold = T, color = "black", background = "#c6c9cf")
...
### fusion_tally 
fuse_tally_tbl <- ggtexttable(fusion_tally, rows = NULL, theme = ttheme("light") ) %>%
...
fusion.in =  fusion.reclass[fusion.reclass$frameness == "in.frame", ]
fusion.in$cancer.subtype = NULL 
fusion.in$patient.id = NULL 
...
in_domain_net = draw_network1(df= in_domain, layout="kk")
render_graph(in_domain_net, layout = "fr")
...
ggplot(testexpfusion, aes(y=value, x=group))+ 
...
balloonplot_type=   ggplot(ant, aes(x =X1, y = X2))  +geom_point( size=0,shape=21, colour="white", fill="#80B1D3", alpha=0 )+
  geom_point( data=ant[ant$value!=0, ], aes(size=value),shape=21, colour="white", fill="#80B1D3", alpha=1) + theme_minimal() +
  geom_tile(color = "black", fill = "transparent") + geom_text(data=ant[ant$value!=0, ], aes(label=value),hjust=.50, vjust=.5, size=12) +
  scale_size_continuous(range = c(50,80)) + theme ( legend.position = "none") + scale_x_discrete(position='top') + xlab("3' Gene") + ylab("5' Gene") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          panel.background = element_blank(), axis.line = element_line(colour = "black")) +
   theme(legend.position="none", legend.title=element_blank(), legend.key = element_blank(),
...
# first are there any diffrences between treated and untreated in terms of numbers of fusion and types! 
#fuse2 = merge ( fuse1[fuse1$frameness == "in.frame", ], rna.key[ , c("sex","sample.type" , "treatment","RNAseq.id")] , by="RNAseq.id")
...
fuse.by.treatment = ggpval:: add_pval(fuse.by.treatment, pairs = list(c(1, 2)), test='wilcox.test')
...
fuse.by.class_plot = ggpval::add_pval(fuse.by.class_plot, pairs = list(c(1, 2)), test='wilcox.test')
...
this.down = this.down[ 
                     (this.down$diff_xcell < normal_check & this.down$tree.xcell == "OUTLIER_LOW" )|
                       (this.down$diff_txns < normal_check & this.down$tree.TxnSpear2 == "OUTLIER_LOW" ) 
                   , ]
...
daf_vs_raf = ggplot(cosmic, aes(x=TUMOR.AF, y= RNA_ALT_AF)) + 
  ...
  scale_color_manual(values = rafdaf_color_simple  , labels = c(HIGH="RAF > DAF", LOW="RAF < DAF", no.sig="not significant")      ) +
...
cosmic_simp = cosmic 
cosmic_simp$balance = gsub ( "EXTREME_", "", cosmic_simp$balance )
rafdaf_color2 = rafdaf_color
rafdaf_color2[ "HIGH"] = rafdaf_color2[ "EXTREME_HIGH"] 
rafdaf_color2[ "LOW"] = rafdaf_color2[ "EXTREME_LOW"] 

total_snv_freq = data.frame (  table ( cosmic_simp$balance )   )
total_snv_freq$label = paste0 ( total_snv_freq$Var1, " (", total_snv_freq$Freq, ")"  )

#cosmic_simp$balance = factor ( cosmic_simp$balance , c ( "HIGH", "LOW", "no.sig"))

final_rafdaf = ggplot(cosmic_simp, aes(x=TUMOR.AF, y= RNA_ALT_AF)) + 
  geom_point( aes ( color=balance , alpha=balance  ), position=position_jitter(h=0.01, w=0.01),  size=3  )  + 
  scale_color_manual(values = rafdaf_color2  , labels = c( LOW=total_snv_freq[total_snv_freq$Var1 == "LOW", ]$label 
                                                           , HIGH=total_snv_freq[total_snv_freq$Var1 == "HIGH", ]$label 
                                                           , no.sig= paste0(" not significant (", total_snv_freq[total_snv_freq$Var1 == "no.sig", ]$Freq, ")"  )
                                                           )      
                     ) +
  theme_minimal()   + xlab("DAF") + ylab ( "RAF") +
  theme(legend.position="right",  legend.key = element_blank(),
        ...
        axis.title.y     = element_text(size=20), 
        legend.text      =element_text(size=12)
  ) + scale_alpha_manual(values=c(  HIGH=.6, LOW=.6,  no.sig=.05  )) +
  guides(
    shape = guide_legend(title.position="top", title.hjust = 0.5, override.aes = list(size=10) , title ="cancer gene")
    , size = FALSE
    #, fill = guide_legend(title.position="top", title.hjust = 0.5)
