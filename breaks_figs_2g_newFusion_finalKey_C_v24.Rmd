...
fuse.by.class_plot = ggpval::add_pval(fuse.by.class_plot, pairs = list(c(1, 2)), test='wilcox.test', textsize=20)
...
psb_results1[which ( row.names ( psb_results1) == "both") , ] = psb_results1[which ( row.names ( psb_results1) == "both") , ] / 2
...
long_cmp_fusion <- temps %>% group_by(type.x, pid) %>% summarise ( TotalbyType = n()
                                                                         #,type.x = factor(type.x, levels=c("primary_only", "both", "secondary_only"))
                                                                         ) %>%
...
long_cmp_fusion = my_add_pval(long_cmp_fusion, pairs = list(c(1, 2)  ), test='wilcox.test', post=2, textsize = 10)
long_cmp_fusion = my_add_pval(long_cmp_fusion, pairs = list(c(2, 3)  ), test='wilcox.test', post=2, textsize = 10) 
...
tally = data.frame ( table ( psb_fusions[ , c("type.x", "is.cancer")]) ) 
 ...
 make.pie( tally[ tally$is.cancer == "yes", ]
                , name.first="3' Gene"
                , cc = c("#8be386" ,"#eb4034", "#79c4d1" )
                , leg.pos = "bottom"
                , title = "3' gene is Cancer"
                )
